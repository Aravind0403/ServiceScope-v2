...
\subsection{Historical CI Validation}
\label{sec:historical}

To assess \textsc{ServiceScope}'s suitability for pre-merge CI integration,
we retrospectively applied the deterministic linker pipeline to 15 historical
pull requests from \emph{microservices-demo}. For each PR, we checked out the
repository at the pre-merge commit state, ran the extraction and manifest
linker pipeline to reconstruct the dependency graph, computed the transitive
blast radius of the modified service, and recorded the result.

\begin{table}[t]
\caption{Historical PR validation on \emph{microservices-demo}. Blast radius
  size is the transitive closure of affected services from the modified
  service. Infrastructure PRs (Terraform, Helm, CI/CD config) are correctly
  identified as having no service-level impact.}
\label{tab:history}
\centering
\small
\begin{tabular}{llcl}
\toprule
PR & Primary Service & Blast Radius & Notes \\
\midrule
3285 & frontend        & 9 & Hub node: all downstream services \\
3281 & checkout        & 7 & Multi-dependency orchestrator \\
3317 & productcatalog  & 3 & \\
3291 & productcatalog  & 3 & \\
3280 & productcatalog  & 3 & \\
3332 & payment         & 2 & \\
3331 & currency        & 2 & \\
3324 & cart            & 2 & \\
3289 & cart            & 2 & \\
3290 & shipping        & 2 & \\
3322 & ad              & 1 & Single downstream caller \\
3288 & shoppingassistant & 0 & Leaf node, no outbound calls \\
\midrule
3326 & terraform       & -- & Skipped: infrastructure \\
3321 & helmchart       & -- & Skipped: infrastructure \\
3329 & .github         & -- & Skipped: CI/CD config \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:history} summarises the results. All 15 pipeline runs
completed successfully. Four PRs (27\%) touching non-service directories
were correctly identified as having no service-level impact---zero false
positive blast radius alerts for infrastructure changes. For the 11 service
PRs, blast radius sizes are consistent with the known architecture: frontend
(the hub node calling all 9 downstream services) produces the largest blast
radius; checkout (orchestrating 7 backend calls) is second; leaf nodes
such as \texttt{shoppingassistant} (no outbound gRPC calls) correctly
return an empty blast radius. All runs completed in under 500\,ms, confirming
suitability for synchronous pre-merge CI integration.

A limitation of this evaluation is that all 15 PRs touched a single service
directory. Co-change correlation---validating whether services predicted in
the blast radius were also modified in the same PR---was therefore not
measurable in this dataset. Such validation would require PRs with
coordinated multi-service changes, which are less common in the squash-merge
...
\textbf{Protocol and packaging coverage.} \textsc{ServiceScope} targets
HTTP and gRPC call sites in source code. SOAP/WS-* services built on
JAX-WS or WCF use generated stub invocations whose endpoint addresses
are bound in WSDL descriptors or Spring XML configuration files outside
the scope of the current manifest linker; extending to SOAP requires
additional descriptor parsers. EAR-packaged enterprise deployments consist
of compiled bytecode rather than source files and require bytecode-level
call graph analysis (e.g., Soot, ASM), which is explicitly outside the
scope of this work. These constraints are consistent with the system's
focus on modern containerised microservice architectures.
