...
async def create_tenant(
    ...
    Requires the `X-Bootstrap-Secret` header to match the `BOOTSTRAP_SECRET`
    environment variable.  Set `BOOTSTRAP_SECRET` to a strong random value for
    first-run bootstrapping; leave it unset (None) to disable this endpoint
    after initial setup.

    Args:
        tenant_data: Tenant creation data
        x_bootstrap_secret: Bootstrap secret from request header
        db: Database session
    ...
    if not settings.BOOTSTRAP_SECRET:
        raise HTTPException(
            ...
            detail="Tenant creation is disabled. Set BOOTSTRAP_SECRET to enable it.",
        )
